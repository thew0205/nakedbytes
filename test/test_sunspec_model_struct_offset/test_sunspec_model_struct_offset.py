...
model_def_file = f"{script_dir}/sunspec_model_struct_offset.json"
...
with open(f"{script_dir}/sunspec_model_generated_struct_offset.h", 'wt') as f:
    f.write(generate_cpp_code(types_desc,root_type_name))
      
with open(f"{script_dir}/model_1.json", 'r') as f:
    ...
    with open(f"{script_dir}/model_1_struct_offset.bin", 'wb') as f:
        f.write(bin)
