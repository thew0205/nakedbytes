...
model_def_file = f"{script_dir}/simple_struct_example.json"
...
with open(f"{script_dir}/simple_struct_example_generated_class.h", 'wt') as f:
    f.write(generate_cpp_code(types_desc,root_type_name))
      
with open(f"{script_dir}/simple_struct_example_data.json", 'r') as f:
    ...
    with open(f"{script_dir}/simple_struct_example.bin", 'wb') as f:
        f.write(bin)
